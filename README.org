...
- Quite fast with multi-threading support using high performance library [[https://software.intel.com/content/www/us/en/develop/tools/oneapi/components/onemkl.html#gs.8jsfgz][MKL]] or [[https://www.openblas.net/][OpenBLAS]] as back-end.
...
There are pre-compiled program provided for both Mac and Linux platform.  If you want to use PCAone with MKL threading, you need to make sure runtime library =libiomp5= can be found. Please check the releases page to download them.
...
make
#+end_src
Since the default is without external MKL or OpenBLAS support, you need to set the =MKLROOT= or =OPENBLAS_ROOT= in *Makefile* to your own path to use one of them as back-end. If it complains that there is no =lapack= lib, please install it and set the =LAPACK_ROOT= to the correct path as well. 
...
*Warning*: On most OS it is very *important* to limit the number of threads to the number of physical cores. 
