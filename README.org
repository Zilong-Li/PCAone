...
the usage since *v0.4.8* =Note:= you must ensure the new plink files contain
*exactly the same SNPs and allele's order*. Below is command using example
data to project new target samples onto the reference coordinates.
