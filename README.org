...
#+begin_src shell
# running out-of-core using only 2GB memory
PCAone --bfile examples/asia -k 20 -h -n 10 -o out -m 2
#+end_src

- run PCA on *scRNAs* data
#+begin_src shell
# assume data is already normalized
PCAone --csv examples/test.csv.zst -k 20 -a -n 10 -o out -m 2 -v
# apply normalization by count per median(CPMED) use --cpmed option
PCAone --csv examples/test.csv.zst -k 20 -a -n 10 -o out -m 2 -v --cpmed
#+end_src
