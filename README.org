#+TITLE: Principal Component Analysis All in One (v0.7.2)
...
Addtionally, an exact PCA (--svd 3) eigendecomposes the N x N sample GRM, streamed from disk
block by block when N <= M, so it needs N^2 memory rather than N x M. Also,
...
- [[#features][Features]]
- [[#whats-new-in-version-about-070][What's new in version about 0.7.0]]
...
- [[#documentation][Documentation]]
  ...
  - [[#ancestry-adjusted-ld][Ancestry adjusted LD]]
...
* What's new in version about 0.7.0 
...
#+begin_src shell
## --USV: prefix to .eigvecs, .eigvals, .loadings, .mbim
...
Use =--ld-stats 1= without =-P= for the standard LD instead. See
[[./docs/ld-ancestry-adjusted.md][docs/ld-ancestry-adjusted.md]] for why these residuals equal the old
=G - USV'= and how they are computed.

*NB:* before v0.8.0 this took a =.residuals= file, written by =-D/--ld= and read
by =-B/--binary=. Both options are removed. =-D/--ld= also ran the PCA without
standardizing the sites; add =--scale 0= to the PCA above to get the same PCs,
and hence the same R2, as before.
