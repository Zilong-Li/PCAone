...
Input options:
  ...
  --pgen-list arg                file with one PLINK2 .pgen/.pvar/.psam prefix per line; used with --make-perm-pgen.
  --make-perm-pgen               merge prefixes from --pgen-list and write a permuted PLINK2 .pgen/.pvar/.psam dataset, then exit.
...
*NB:* Make sure the first allele in BEAGLE is the ref allele in the reference
samples! Use angsd --doMajorMinor 3 option to produce the BEAGLE file.

** GENOME-wide selection scan

PCAone can test selection for differentiated variants after a good reference
PCA has been computed with good sites after site-level QC, e.g. LD prunning.
...
For chromosome-sharded biobank data, create a text file with one PGEN prefix per
line, then merge and physically permute variants into a single PCAone-ready
basic hard-call PGEN dataset.

#+begin_src shell
./PCAone --pgen-list ukb_pgen_prefixes.txt --make-perm-pgen -o ukb.merged.perm --seed 112
./PCAone --pgen ukb.merged.perm -k 20 -m 8 --hardcall -S
#+end_src

All input shards must contain identical =.psam= files in the same sample order.
The preprocessing step uses =--buffer= as the approximate in-memory output block
size in GB; larger values improve read locality during physical permutation when
enough RAM is available.
The preprocessing output uses PLINK2 basic hard-call PGEN encoding, so dosage
tracks from the source files are converted to hard calls and are not preserved
in the merged file.
