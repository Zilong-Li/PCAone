...
represents a feature and each col represents a PC. need to use =--printv=
option to print it.
...
*** run the window-based RSVD method (algorithm2, default) with in-core mode
#+begin_src shell
...
ancestry-adjusted LD statistics to account for the population structure in
LD. See our [[https://doi.org/10.1101/2024.05.02.592187][paper]] for more details.

To calculate the ancestry-adjusted LD matrix, we need to specify the number
of principal components (=-k/--pc=), which can be thought of the number of
distinct ancestral groups and can capture the population structure. In this
example, we calculate and output the ancestry adjusted LD matrix in a file
named =adj_ld.residuals=, assuming that 3 PCs can accout for population
structure.

#+begin_src shell
./PCAone -b example/plink -k 3 -m 2 --ld -o adj_ld
#+end_src
...
We want to compute the top 10 PCs for this genotype dataset using 4
threads and only 2GB memory. We will use the proposed window-based RSVD
algorithm with default setting =--svd 2=.

#+begin_src shell
./PCAone --bfile example/plink -k 5 -m 2
#+end_src
