...
if [ $platform == "Darwin" ];then
    ...
    make -f linux.makefile clean && make -f linux.makefile 2>/dev/null && zip PCAone_Linux_noiomp5.zip PCAone && echo "Publishing PCAone_Linux.zip done";

    awk 'NR==9 {$0="#"$0}; NR==7 {sub("# ", "")}; NR==16 {sub("0", "1")}; 1' Makefile >linux2.makefile
    make -f linux2.makefile clean && make -f linux2.makefile 2>/dev/null && zip PCAone_Linux.zip PCAone && echo "Publishing PCAone_Linux_iomp5.zip done";
