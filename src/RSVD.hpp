...
struct StandardNormalRandomHelper<MatrixType, RealType<MatrixType>,
                                  RandomEngineType> {
  static inline MatrixType generate(const Eigen::Index numRows,
    ...
    const auto normal{
        [&](typename MatrixType::Scalar) { return distribution(engine); }};
...
template <typename MatrixType, typename RandomEngineType>
struct StandardNormalRandomHelper<
    MatrixType, std::complex<RealType<MatrixType>>, RandomEngineType> {
  static inline MatrixType generate(const Eigen::Index numRows,
                                    const Eigen::Index numCols,
                                    RandomEngineType& engine) {
    ...
    const auto complexNormal{[&](typename MatrixType::Scalar) {
      return std::complex<RealType<MatrixType>>(distribution(engine),
                                                distribution(engine));
    }};
...
inline MatrixType StandardNormalRandom(const Eigen::Index numRows,
  ...
  return StandardNormalRandomHelper<MatrixType, typename MatrixType::Scalar,
                                    RandomEngineType>::generate(numRows,
                                                                numCols,
                                                                engine);
}

template <typename MatrixType>
inline void permute_matrix(
    MatrixType& G, Eigen::PermutationMatrix<Eigen::Dynamic, Eigen::Dynamic>& P,
    bool bycol = true) {
  if (bycol) {
    P = Eigen::PermutationMatrix<Eigen::Dynamic, Eigen::Dynamic>(G.cols());
    P.setIdentity();
    auto rng = std::default_random_engine{};
    std::shuffle(P.indices().data(), P.indices().data() + P.indices().size(),
                 rng);
...
class RsvdOpOnePass {
 ...
 public:
  ...
  RsvdOpOnePass(ConstGenericMatrix& mat_, int k_, int os_ = 10, int rand_ = 1)
    ...
    if (rand == 1) {
      Omg = StandardNormalRandom<MatrixType, std::default_random_engine>(
          ncol, size, randomEngine);
    } else {
      Omg = UniformRandom<MatrixType, std::default_random_engine>(ncol, size,
                                                                  randomEngine);
    }
  }
    ...
    if (p > 0) {
      for (uint32_t pi = 0; pi < p; pi++) {
        if (finder == 1) {
          ...
          Omg.noalias() =
              qr.householderQ() * MatrixType::Identity(cols(), size);
        } else if (finder == 2) {
  ...
  void computeGandH(MatrixType& G, MatrixType& H, uint32_t p,
    ...
    if (windows % 2 != 0)
      throw std::runtime_error(
          "windows must be a power of 2, ie. windows=2^x.\n");
    ...
    for (uint32_t pi = 0; pi <= p; pi++) {
      ...
      for (uint32_t b = 0, i = 1, j = 1; b < windows; ++b, ++i, ++j) {
        ...
        stop_idx =
            (b + 1) * blocksize >= nrow ? nrow - 1 : (b + 1) * blocksize - 1;
        ...
        if (pi > 0 && j <= std::pow(2, pi - 1) && std::pow(2, pi) < windows) {
          if (trans)
            ...
            H1.noalias() +=
                mat.middleRows(start_idx, actual_block_size).transpose() *
                G.middleRows(start_idx, actual_block_size);
...
class RsvdOnePass {
 public:
  ...
  void computeUSV(uint32_t p, uint32_t windows = 0) {
    ...
    {
      Eigen::HouseholderQR<Eigen::Ref<MatrixType>> qr(G);
      Rt.noalias() = MatrixType::Identity(size, nrow) *
                     qr.matrixQR().template triangularView<Eigen::Upper>();
      G.noalias() = qr.householderQ() * MatrixType::Identity(nrow, size);
    }
    ...
    Eigen::JacobiSVD<MatrixType> svd(B,
...
class RsvdOne {
 ...
 public:
  RsvdOne(ConstGenericMatrix& mat_, uint32_t k_, uint32_t os_ = 10,
          uint32_t rand_ = 1)
