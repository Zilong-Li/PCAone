...
Param::Param(int argc, char ** argv)
{
    ...
    opts.add<Switch>("C", "cpmed",
                     "normalize values by count per median (CPMED) and log transformation for scRNAs",
                     &cpmed);
    ...
    opts.add<Value<uint64>, Attribute::advanced>("M", "", "number of features (eg. SNPs) if already known", 0,
                                                 &nsnps);
    ...
    opts.add<Value<uint>, Attribute::advanced>("", "buffer", "buffer in GB uint used for permuting the data",
                                               buffer, &buffer);
    opts.add<Value<uint>, Attribute::advanced>(
        "", "center", "do centering for csv input file. 0: disabled, 1: enabled", center, &center);
    opts.add<Value<uint>, Attribute::advanced>("", "imaxiter", "maximum number of IRAM interations", imaxiter,
                                               &imaxiter);
    ...
    opts.add<Value<uint>, Attribute::advanced>("", "ncv", "number of Lanzcos basis vectors for IRAM", ncv,
                                               &ncv);
    opts.add<Value<uint>, Attribute::advanced>("", "oversamples", "number of oversampling columns for RSVD",
                                               oversamples, &oversamples);
    opts.add<Value<uint>, Attribute::advanced>("", "rand", "the random matrix type. 0: uniform, 1: guassian",
                                               rand, &rand);
    opts.add<Value<double>, Attribute::advanced>("", "tol", "tolerance for RSVD algorithm", tol, &tol);
    opts.add<Value<double>, Attribute::advanced>("", "tol-em", "tolerance for EMU/PCAngsd algorithm", tolem,
                                                 &tolem);
    opts.add<Value<double>, Attribute::advanced>("", "tol-maf", "tolerance for MAF estimation updated by EM",
                                                 tolmaf, &tolmaf);
    ...
    try
    {
        ...
        if(intype == FileType::BEAGLE) pcangsd = true;
        if(emu || pcangsd)
        {
            runem = true;
        }
